...
barplot(table(data$labelvideolist), col="cornflowerblue", cex.axis=0.6, cex.names=0.3, cex.lab=0.8,
...
```{r, echo=FALSE, fig.height=4, cache=FALSE}
...
We can see that configurations of subjects are very different. Consequently, it makes the comparaisons of the videos quite complicated.
...
## Raw data
```{r, cache=FALSE}
rawdatafather <- data[which(data$indexList=="F1044C1"),]$father
rawdataMother <- data[which(data$indexList=="F1044C1"),]$mother
rawdataTherapist <- data[which(data$indexList=="F1044C1"),]$therapist
