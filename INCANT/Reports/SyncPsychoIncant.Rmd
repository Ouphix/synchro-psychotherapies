...
data$father <- data$father + min(data$father[which (data$father >0)])/2
data$logFather <- log(data$father)

#log(x -min(x)+1)
data$mother <- data$mother + min(data$mother[which (data$mother >0)])/2
data$logMother <- log(data$mother)

data$patient <- data$patient + min(data$patient[which (data$patient >0)])/2
data$logPatient <- log(data$patient)

data$therapist <- data$therapist + min(data$therapist[which (data$therapist >0)])/2
...
# If the colum is not empty, takes its length and begin a data frame with it
      ...
      write.csv(dfSliding, paste("/Users/Ofix/Documents/Fac/internat/Recherche/projets/synchro/synchroData/Git/INCANT/Data/CSV/MotionHistory/filtered/noLog/",videoName, ".slideddata.csv", sep=""))
