...
output: 
  pdf_document: 
    toc: yes
---
```{r Clean Environement, echo=FALSE}
...
### MeanMotionByTime
Function that takes raw motion history data and computes the mean on a given interval. Intervals don't overlap, so the frequency of the data change (from 25 frames by seconde to 25 frames/interval by second).
...
```{r raw Global motion history by situation : "conflict" vs "no conflict", fig.height=2}
...
### Sex
```{r Demographic description, eval=FALSE}
par(mar=c(3,4,4,4))
...
## Insecurity level
```{r Insecurity level, fig.height=4}
par(mar=c(9,5,3,3))
barplot(table(psycho$insecurite_level), las=2)
```

## TAS
```{r TAS, fig.height=2.5}
par(mar=c(3,3,3,3))
