...
boxplot(data$father, data$mother, data$child, 
        ...
        main= "Motion history by fra me box plots (raw data), all videos", las=1, ylab="Motion history")
par(mar=c(1,0.5,0.5,1))
...
# Global Synchrony scores no conflict vs conflict (log data)
```{r SSIlog}
str(SSIlog)
str(cutFrames)
View(cutFrames)
...
Between is the conflictual discussion, numeric variable in minutes
* **CutFinalMin** : when the experimenter come back to shut down the camera, numeric variable in minutes
* **LabelVideo** : label of the video if it is related to No Conflict period or Conflict Perido
...
hist(log(SSInoLog$SSI))

hist(ncSSI)
hist(cSSI)
hist(log(SSInoLog[which(SSInoLog$LabelVideo=="No Conflict"),]$SSI))
hist(log(SSInoLog[which(SSInoLog$LabelVideo=="Conflict"),]$SSI))

SSImean <- data.frame(ncSSI, cSSI)
SSImean
barplot(as.matrix(SSImean))
